...
function PublicPage({ locale, page, data }: { locale: PublicLocale; page: PublicPage; data: PublicSiteData }) {
  ...
  if (page === "home") return <>
    <section className="public-home-hero">
      ...
      <div className="public-home-hero__content"><span>{copy.hero.kicker}</span><h1>{data.profile?.name ?? copy.hero.title}</h1><p>{data.profile?.tagline ?? copy.hero.copy}</p><div><Link className="public-cta" href={publicHref(locale, "registration")}>{copy.hero.primary}<ArrowRight aria-hidden="true" size={19} /></Link><Link className="public-cta public-cta--quiet" href={publicHref(locale, "programs")}>{copy.hero.secondary}</Link></div></div>
    </section>
  ...
  return <div className="public-page"><PageIntro eyebrow={copy.contact.eyebrow} title={copy.contact.title} intro={copy.contact.copy} /><section className="public-contact"><Image src="/dar-al-hadith/facade.webp" alt="" width={760} height={520} /><div>{data.profile?.phone && <a href={`tel:${data.profile.phone}`}><Phone aria-hidden="true" /><span><small>{copy.contact.phone}</small><strong>{data.profile.phone}</strong></span></a>}{data.profile?.address && <p><MapPin aria-hidden="true" /><span><small>{copy.contact.address}</small><strong>{data.profile.address}</strong></span></p>}{data.profile?.mapUrl && <a className="public-cta" href={data.profile.mapUrl} target="_blank" rel="noreferrer">{copy.contact.map}<ExternalLink aria-hidden="true" size={18} /></a>}</div></section></div>;
}
