...
function scheduleLabel(sessions: StudentDashboardData["courseSchedule"]): string { const days = ["الأحد", "الإثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت"]; const labels = sessions.map((session) => session.dayOfWeek === undefined ? `${new Intl.DateTimeFormat("ar-MA", { weekday: "long" }).format(new Date(session.startsAt))} · ${timeRange(session.startsAt, session.endsAt)}` : `${days[session.dayOfWeek]} · ${session.startsAt.slice(0,5)}–${session.endsAt.slice(0,5)}`); return [...new Set(labels)].join("، ") || "—"; }
