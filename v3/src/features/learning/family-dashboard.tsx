...
export function FamilyDashboard({ accounts, data }: { accounts: FamilyChildSummary[]; data: FamilyChildData | null }) {
  ...
  return <div className="learning-page family-dashboard">
    ...
    <div className="family-sections">
      ...
      <FamilySection id="messages" title="الإعلانات والرسائل" icon={Megaphone}>{data && (data.messages.length || data.announcements.length) ? <>{data.messages.map((item) => <Line key={item.id} title={item.title} meta={item.body} badge={item.read ? item.createdAt ? formatDate(item.createdAt) : item.dateLabel ?? "رسالة سابقة" : "جديد"} />)}{data.announcements.map((item) => <Line key={item.id} title={item.title} meta={item.body} badge={formatDate(item.publishedAt)} />)}</> : <SmallEmpty text="لا توجد رسائل جديدة" />}</FamilySection>
