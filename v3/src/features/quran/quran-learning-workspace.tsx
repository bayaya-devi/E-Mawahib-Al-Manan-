...
export function QuranLearningWorkspace({ surah }: { surah: QuranSurah }) {
  ...
  const start = phase.from - 1;
  const finish = phase.to;
  ...
  return <div className="quran-workspace student-surah-workspace">
    ...
    {audioStep ? <>
      ...
      <div className="student-learning-actions"><ButtonLink href="/student/quran" variant="quiet">رجوع</ButtonLink><Button aria-label={phaseIndex < phases.length - 1 ? "المرحلة التالية" : "متابعة"} onClick={() => { audio.current?.pause(); if (phaseIndex < phases.length - 1) { const next = phaseIndex + 1; setPhaseIndex(next); setCurrent(phases[next]!.from - 1); setFallback(false); setPlayAll(true); } else setAudioStep(false); }}>{phaseIndex < phases.length - 1 ? "المرحلة التالية" : "بدء التمارين"}</Button></div>
    </> : null}
    <div hidden={audioStep}><LearningExercise learningKey={`surah-${surah.number}`} rewardStars={surah.verseCount <= 5 ? 3 : surah.verseCount <= 10 ? 4 : 5} onState={onState} /></div>
