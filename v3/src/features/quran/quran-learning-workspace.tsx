...
export function QuranLearningWorkspace({ surah }: { surah: QuranSurah }) {
  ...
  const onState = useCallback((state: LearningState, finalStart: number) => {
    if (Math.floor(lastCursor.current / 4) !== Math.floor(state.cursor / 4) && state.cursor < finalStart) {
      setCurrent(0); setFallback(false); setPlayAll(true); setAudioStep(true);
    }
    if (state.cursor >= finalStart) setAudioStep(false);
    lastCursor.current = state.cursor; setCursor(state.cursor); setCompleted(state.passed);
  }, []);
  useEffect(() => {
    const player = audio.current; if (!player) return;
    let live = true;
    if (!audioStep || finalStep) return;
    player.load();
  ...
  return <div className="quran-workspace student-surah-workspace">
    ...
    <div hidden={audioStep && !finalStep}><LearningExercise learningKey={`surah-${surah.number}`} onState={onState} /></div>
