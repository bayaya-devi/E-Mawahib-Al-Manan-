...
type Juz = { number: number; name: string; from: number; to: number };
const JUZ: readonly Juz[] = [
...
export function QuranCatalog({ progress }: { progress: StudentDashboardData["progress"] }) {
  ...
  return <div className="student-quran-path">{JUZ.map((juz) => {
    ...
    return <section className={`juz-path ${active ? "is-active" : ""} ${complete ? "is-complete" : ""} ${future ? "is-locked" : ""}`} key={juz.number}>
      <button type="button" className="juz-path__head" onClick={() => setOpenJuz((value) => value === juz.number ? null : juz.number)} aria-expanded={expanded}>
...
function SurahNode({ surah, item, unlocked }: { surah: ReturnType<typeof getAllSurahs>[number]; item: StudentDashboardData["progress"][number] | undefined; unlocked: boolean }) {
  ...
  const content = <><span className={`surah-node__dot is-${status}`}>{status === "mastered" ? <Check size={16} /> : unlocked ? <BookOpenText size={16} /> : <LockKeyhole size={15} />}</span><span className="surah-node__line" /><span className="surah-node__card"><strong>{surah.nameArabic}</strong><small>{surah.verseCount} آيات</small>{status === "mastered" ? <em>مكتملة</em> : status === "in_progress" ? <em>تابع</em> : null}</span></>;
