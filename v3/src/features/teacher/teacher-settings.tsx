...
import { readDeviceAccounts, rememberAuthenticatedAccount, removeDeviceAccount } from "./device-account-vault";
...
export function TeacherSettings() {
  ...
  async function switchAccount(account: SavedDeviceAccount) { if (account.id === activeId) return; setBusy(true); const result = await createClient().auth.setSession({ access_token: account.accessToken, refresh_token: account.refreshToken }); setBusy(false); if (result.error || !result.data.session) { removeDeviceAccount(account.id); setAccounts(readDeviceAccounts()); return showToast({ title: "انتهت جلسة هذا الحساب", description: "سجل(ي) الدخول إليه من جديد.", tone: "info" }); } router.push(roleHome(account.kind)); router.refresh(); }
