...
export async function getMessagingWorkspace(): Promise<MessagingWorkspaceData> {
  ...
  try {
    ...
    const [conversationResult, memberResult, messageResult, targetResult, requestEventResult] = await Promise.all([
      ...
      client.rpc("list_message_targets"),
    ...
    const candidateIds = (targetResult.data ?? []).map((item) => item.user_id);
    ...
    return {
      ...
      messages,
      targets: (targetResult.data ?? []).map((item) => ({ id: item.user_id, name: item.display_name })),
