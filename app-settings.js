const MawahibSettings = (() => {
  ...
  function init() { if (typeof Auth === 'undefined' || !Auth.getSession()) return; injectStyles(); injectButton(); injectPanel(); renderPanel(); bindNavButtons(); initNavAutoHide(); }
  function injectStyles() { if (document.getElementById('mawahib-settings-style')) return; const style = document.createElement('style'); style.id = 'mawahib-settings-style'; style.textContent = [
    ...
    '.theme-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}', '.theme-choice{border:1px solid #e5e7eb;border-radius:16px;padding:10px;font-weight:900;background:#fff;cursor:pointer;text-align:center}', '.theme-choice.active{background:linear-gradient(135deg,var(--platform-primary,#14532d),var(--platform-primary-dark,#0c4a3b));color:#fff;border-color:transparent}', '.nav-bottom{transition:transform .24s ease}', '.nav-bottom.mawahib-nav-hidden{transform:translateY(calc(100% + env(safe-area-inset-bottom)))}', '@media(max-width:640px){.mawahib-settings-fab{top:auto;bottom:86px;left:12px}.theme-grid{grid-template-columns:1fr}}'
  ].join('\n'); document.head.appendChild(style); }
  function injectButton(){ if(document.getElementById('mawahib-settings-button') || document.querySelector('[data-settings-toggle]')) return; const button=document.createElement('button'); button.id='mawahib-settings-button'; button.className='mawahib-settings-fab'; button.type='button'; button.title='الإعدادات'; button.setAttribute('aria-label','الإعدادات'); button.textContent='⚙'; button.addEventListener('click',open); document.body.appendChild(button); }
  function bindNavButtons(){ document.querySelectorAll('[data-settings-toggle]').forEach(button=>{ if(button.dataset.settingsBound==='1') return; button.dataset.settingsBound='1'; button.addEventListener('click',open); }); }
  function initNavAutoHide(){
    ...
    let revealTimer;
    const hide=()=>{
      nav.classList.add('mawahib-nav-hidden');
      clearTimeout(revealTimer);
      revealTimer=setTimeout(()=>nav.classList.remove('mawahib-nav-hidden'),520);
    };
  ...
  function accountRow(account,current){ const active=current && current.username===account.username; const role=account.role==='prof'?'أستاذ':'تلميذ'; return '<button type="button" class="account-row '+(active?'active':'')+'" data-account-row="'+escapeAttr(account.username)+'" onclick="MawahibSettings.switchAccount(\''+escapeAttr(account.username)+'\')"><span style="flex:1"><strong style="display:block;color:#111827">'+escapeHtml(account.prenom||'')+' '+escapeHtml(account.nom||'')+'</strong><span style="font-size:11px;color:#64748b;font-weight:900">'+role+' · '+escapeHtml(account.username)+'</span></span><span class="settings-action secondary">فتح</span></button>'; }
  ...
  async function switchAccount(username){
    if(!Auth.switchAccount) return;
    const row=[...document.querySelectorAll('[data-account-row]')].find(item=>item.dataset.accountRow===username);
    if(row){ row.disabled=true; row.style.opacity='.72'; const action=row.querySelector('.settings-action'); if(action) action.textContent='جار الفتح'; }
