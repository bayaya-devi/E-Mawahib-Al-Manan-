(function(){
  ...
  function validVerseRange(surah,scope,start,end){if(scope!=='جزء')return true;const first=Number(start);const last=Number(end);const max=Number(surahAyat(surah));return Number.isInteger(first)&&Number.isInteger(last)&&first>=1&&last>=first&&last<=max}
